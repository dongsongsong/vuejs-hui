<?php if (!defined('THINK_PATH')) exit();?><html><head>
  ...
  <link href="/Public/workerman/css/bootstrap.min.css" rel="stylesheet">
  <link href="/Public/workerman/css/style.css" rel="stylesheet">
  <link href="/Public/workerman/css/biaoqing.css" rel="stylesheet">
  <!-- Include these three JS files: -->
  <script type="text/javascript" src="/Public/workerman/js/swfobject.js"></script>
  <script type="text/javascript" src="/Public/workerman/js/web_socket.js"></script>
  <script type="text/javascript" src="/Public/workerman/js/json.js"></script>
  <script type="text/javascript" src="http://www.phpernote.com/js/jquery.min.js"></script>
  <script type="text/javascript" src="/Public/workerman/js/jquery.qqFace.js"></script>
  <script type="text/javascript">
  $(function(){
		$('.emotion').qqFace({
			id : 'facebox', //表情盒子的ID
			assign:'textarea', //给那个控件赋值
			path:'/Public/workerman/img/face/'	//表情存放的路径
		});
	});
	//查看结果
	function replace_em(str){
		...
		str = str.replace(/\>/g,'&gt;');
		str = str.replace(/\n/g,'<br/>');
		str = str.replace(/\[em_([0-9]*)\]/g,"<img src='/Public/workerman/img/face/$1.gif' border='0' />");
		return str;
	}
    ...
    function init() {
      ...
      ws.onmessage = function(e) {
        ...
        switch(data['type']){
             ...
             // 用户退出 更新用户列表
              case 'logout':
            	  // alert(data['from_client_name']+"退出了。。");
    ...
    function onSubmit() {
      ...
      var data = JSON.stringify({"type":"say","to_client_id":to_client_id,"to_client_name":to_client_name,"content":content});
      ws.send(data);
      input.value = "";
      input.focus();
    }
